...
$CLI_EXECUTABLE createsubca --issuerdn "${TEST_HTTPS_ROOT}" --dn "${TEST_HTTPS_SERVER}"  --signalgo SHA512RSA --keyalgo RSA --keysize 2048  --validity 10/10/2012-10/10/2014
$CLI_EXECUTABLE exportcert  --dn "${TEST_HTTPS_ROOT}"
$CLI_EXECUTABLE exportcert  --dn "${TEST_HTTPS_SERVER}"

x509=`openssl x509 -in CA_DEVEL_HTTPS_SERVERS_cert.pem  -noout -text`
#check version
test_or_print_error 'echo $x509 |grep -q "Version: 3 (0x2)"'\
	"Invalid Version"
#check subject
test_or_print_error 'echo $x509 |grep -q "Subject: C=FR, ST=France, L=Paris, O=FOO_O, OU=FOO_OU, CN=FOO_CN_SERVER"' \
	"Invalid Subject"
#check issuer
...
test_or_print_error 'openssl verify  -CAfile CA_DEVEL_HTTPS_ROOT_cert.pem  CA_DEVEL_HTTPS_SERVERS_cert.pem > /dev/null' \
